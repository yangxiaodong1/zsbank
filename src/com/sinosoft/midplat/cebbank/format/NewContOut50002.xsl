...
<xsl:stylesheet version="1.0" 
	...
	<xsl:template name="Transaction_Body" match="Body">
		<Body>
			...
			<PRINT>
				...
				<PRINT_LINE/>
				<PRINT_LINE/>
				<PRINT_LINE><xsl:text>　　　保险单号码：</xsl:text><xsl:value-of select="ContNo" /><xsl:text>-(00001～</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(concat($ContPlanMultWith0,')'),5)" /><xsl:text>                                      币值单位：人民币元</xsl:text></PRINT_LINE>
				...
				<PRINT_LINE><xsl:text>　　　银行网点名称：</xsl:text><xsl:value-of select="AgentComName"/></PRINT_LINE>
				<PRINT_LINE><xsl:text>　　　银行销售人员：</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(TellerName,49)"/><xsl:text>银行销售人员工号：</xsl:text><xsl:value-of select="SellerNo"/></PRINT_LINE>
			</PRINT>
			...
			<PRINT>
			    <PRINT_LINE>AAAAAAAAAA</PRINT_LINE><!-- 光大银行的特殊约定，十个A表示换页 -->
			    <PRINT_LINE/>
				<PRINT_LINE/>
				<PRINT_LINE/>
				<PRINT_LINE/>
	...
	<xsl:template name="tran_riskcode">
		...
		<xsl:choose>
			...
			<!-- 安邦聚宝盆2号两全保险（分红型）A款 -->
			<xsl:when test="$riskcode='122008'">122008</xsl:when>
			<!-- 安邦白玉樽1号终身寿险（万能型） -->
