...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="TRAN_BODY" match="Body">
		...
		<MAIN>
			...
			<!-- 承保日期 -->
			<ACCEPT><xsl:value-of select="Risk/PolApplyDate" /></ACCEPT>
			<!-- 首期保费, 大写 -->
			...
			<!-- 生效日期 -->
			<VALIDATE><xsl:value-of select="Risk/CValiDate" /></VALIDATE>
			<!-- 保险终止日期 -->
			<INVALIDATE><xsl:value-of select="Risk/InsuEndDate" /></INVALIDATE>
			<!-- 缴费日期 -->
			<PAYDATECHN><xsl:value-of select="Risk/PolApplyDate" /></PAYDATECHN>
			<!-- 首期缴费日期 -->
			<FIRSTPAYDATE><xsl:value-of select="Risk/PolApplyDate" /></FIRSTPAYDATE>
			<!-- 缴费起止日期 -->
			<PAYSEDATECHN><xsl:value-of select="Risk/PayEndDate" /></PAYSEDATECHN>
			<!-- 缴费年度 -->
			<PAYYEAR></PAYYEAR>
			<!-- 承保公司 -->
		...
		<FILE_LIST>
			...
			<PAGE_LIST>
				...
				<Detail>
					...
					<BKDETAIL></BKDETAIL>
					<BKDETAIL></BKDETAIL>
					<BKDETAIL><xsl:text>　　　保险单号码：</xsl:text><xsl:value-of select="ContNo" /><xsl:text>                                                    币值单位：人民币元</xsl:text></BKDETAIL>
					<BKDETAIL>　　　------------------------------------------------------------------------------------------------</BKDETAIL>
					<BKDETAIL>
					...
					<xsl:for-each select="Risk">
						...
						<BKDETAIL><xsl:text>　　　</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(RiskName, 40)"/>
												<xsl:choose>
					...
					<BKDETAIL>　　　------------------------------------------------------------------------------------------------</BKDETAIL>
					<BKDETAIL></BKDETAIL>
					<BKDETAIL>　　　保险费合计：<xsl:value-of select="PremText" />（RMB<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fenToYuan(Prem)"/>元整）</BKDETAIL>
					<BKDETAIL></BKDETAIL>
					<BKDETAIL>　　　------------------------------------------------------------------------------------------------</BKDETAIL>
					<BKDETAIL></BKDETAIL>
					<BKDETAIL>　　　------------------------------------------------------------------------------------------------</BKDETAIL>
					<xsl:variable name="SpecContent" select="Risk[RiskCode=MainRiskCode]/SpecContent"/>
					<BKDETAIL>　　　保险单特别约定：<xsl:choose>
		...
		<xsl:if test="CashValues/CashValue/EndYear != ''">
