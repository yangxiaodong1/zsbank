...
<xsl:stylesheet version="1.0"
			...
			<xsl:for-each select="Risk[RiskCode=MainRiskCode]">
				<PT>
					...
					<xsl:choose>
						...
						<xsl:otherwise>
							<CHARGE_PERIOD><xsl:apply-templates select="PayEndYearFlag" /></CHARGE_PERIOD>
							<CHARGE_YEAR>
		...
		<FILE_LIST>
			...
			<PAGE_LIST>
				...
				<Detail>
					...
					<BKDETAIL></BKDETAIL>
					<BKDETAIL></BKDETAIL>
					<BKDETAIL><xsl:text>　　　保险单号码：</xsl:text><xsl:value-of select="ContNo" />
				...
				<xsl:for-each select="Risk">
					...
					<BKDETAIL>　　　    《安邦长寿安享3号年金保险计划》由主险《安邦长寿安享3号年金保险》及附加险《安邦附加长寿添利3号</BKDETAIL>
					<BKDETAIL>　　　两全保险（万能型）》组成。经生存年金受益人同意后，主险产品的生存年金，在年金给付日自动转入附加险</BKDETAIL>
					<BKDETAIL>　　　万能账户中，享受日日复利，月月结息的资金增值服务。</BKDETAIL>
					<BKDETAIL>　　　-------------------------------------------------------------------------------------------------</BKDETAIL>
		...
		<FILE_LIST>
			...
			<PAGE_LIST>
				...
				<Detail>
					...
					<BKDETAIL></BKDETAIL>
					<BKDETAIL>　　　保险单号码：<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(ContNo, 60)"/>币值单位：人民币元 </BKDETAIL>
					<BKDETAIL>　　　------------------------------------------------------------------------------------------------</BKDETAIL>
					<BKDETAIL>　　　被保险人姓名：<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(Insured/Name, 49)"/><xsl:text>险种名称：</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_($MainRisk/RiskName,0)"/></BKDETAIL>
					<BKDETAIL><xsl:text/>　　　保单年度末<xsl:text>             </xsl:text>
                        ...
                        <xsl:if test='position() &lt; $n'>
                            <BKDETAIL><xsl:text/><xsl:text>　　　　　</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(concat(position(),''),8)"/><xsl:text>　　　　　 </xsl:text>
                            ...
                            <xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(concat(java:com.sinosoft.midplat.common.NumberUtil.fenToYuan(../CashValue[$n2]/Cash), '元'),0)"/></xsl:if></BKDETAIL>
                        </xsl:if>
						</xsl:for-each>
					<BKDETAIL>　　　------------------------------------------------------------------------------------------------</BKDETAIL>
					<BKDETAIL></BKDETAIL>
	...
	<xsl:template name="tran_risk">
		...
		<xsl:choose>
			<xsl:when test="$riskcode='50011'">50011</xsl:when>
			<xsl:when test="$riskcode='L12068'">L12068</xsl:when><!-- 主险：L12068 - 安邦长寿安享3号年金保险 -->
			<xsl:when test="$riskcode='L12069'">L12069</xsl:when><!-- 附加险：L12069 - 安邦附加长寿添利3号两全保险（万能型） -->	
			<xsl:otherwise>--</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
