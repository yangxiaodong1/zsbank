package com.sinosoft.midplat.grcb.net;
...
public class GrcbNetImpl extends SocketNetImpl {

	// 银行固定标识
	private static final String BANK_FLAG = "GRCB";
	// 编码集
	private static final String GBK_ENCODING = "GBK";
	// 银行交易代码
	private String cOutFuncFlag = null;
	
	public GrcbNetImpl(Socket socket, Element thisConfRoot) throws MidplatException {
	...
	public Document receive() throws Exception {
		...
		cLogger.info("Into GrcbNetImpl.receive()...");
		
		/*
		 * 每个数据包的基本格式为：包头+包长+包体。
		 * 包头：GZCB（4字符）+交易码（4字符）。其中GZCB为标识码，包头总占位为8个字符。
	...
	public void send(Document pOutNoStd) throws Exception {
		...
		cLogger.info("Into GrcbNetImpl.send()...");
		...
		cLogger.info("Out GrcbNetImpl.send()!");
	}
