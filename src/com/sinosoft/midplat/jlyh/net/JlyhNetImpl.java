package com.sinosoft.midplat.jlyh.net;
...
public class JlyhNetImpl extends SocketNetImpl {

	// 银行固定标识
	private static final String BANK_FLAG = "JLYH";
	// 编码集
	private static final String GBK_ENCODING = "GBK";
	// 银行交易代码
	private String cOutFuncFlag = null;
	
	public JlyhNetImpl(Socket socket, Element thisConfRoot) throws MidplatException {
	...
	public Document receive() throws Exception {
		...
		cLogger.info("Into JlyhNetImpl.receive()...");
		
		/*
		 * 每个数据包的基本格式为：包头+包长+包体。
		 * 包头：JLYH（4字符）+交易码（4字符）。其中JLYH为标识码，包头总占位为8个字符。
		...
		String xmlHead = new String(mHeadBytes, GBK_ENCODING);	// 上海农商银行报文编码：GBK
	...
	public void send(Document pOutNoStd) throws Exception {
		...
		cLogger.info("Into JlyhNetImpl.send()...");
		...
		cLogger.info("Out JlyhNetImpl.send()!");
	}
