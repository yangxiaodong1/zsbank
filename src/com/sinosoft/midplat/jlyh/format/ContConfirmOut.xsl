...
<xsl:stylesheet version="1.0" 
	...
	<xsl:template name="Transaction_Body" match="Body">
		<Body>
			...
			<PRINT>
				...
				<PRINT_LINE></PRINT_LINE>
				<PRINT_LINE><xsl:text>　银行网点名称：</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(AgentComName, 45)"/><xsl:text>银行销售人员工号：</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(SellerNo, 20)"/></PRINT_LINE>
			</PRINT>
	...
	<xsl:template name="tran_riskcode">
		...
		<xsl:choose>
			...
			<xsl:when test="$riskcode=L12079">122012</xsl:when>
			<!-- 安邦盛世2号终身寿险（万能型） -->
			<!-- 盛世3号产品代码变更start -->
			<xsl:when test="$riskcode=L12078">122010</xsl:when>
			<xsl:when test="$riskcode=L12100">122010</xsl:when>
			<!-- 盛世3号产品代码变更end -->
			<!-- 安邦盛世3号终身寿险（万能型）  -->
