...
public class PolicyStatusQuery extends ServiceImpl{

	public PolicyStatusQuery(Element thisBusiConf) {
		super(thisBusiConf);
	}
	...
	public Document service(Document inXmlDoc) throws Exception {
		cLogger.info("Into PolicyStatusQuery.service()...");

		long mStartMillis = System.currentTimeMillis();
		// 获取文件名
		tranDate = inXmlDoc.getRootElement().getChild("Body").getChildText("EdorCTDate");
...
//			Element element = (Element)XPath.selectSingleNode(outNoStdXml.getRootElement(), "//TranData/SurrDetails");
			String sumCount = outNoStdXml.getRootElement().getChild("AllNum").getText();
//			JdomUtil.print(element);
//			if(surrDetailList != null && surrDetailList.size()>0){
//				surrCount = surrDetailList.size();
//			}
			surrCount = Integer.parseInt(sumCount);
...
//			JdomUtil.print(content);
		...
		cLogger.info("Out PolicyStatusQuery.service()!");
		return cOutXmlDoc;
	}
...
//		outRootXml.addContent(0, outRootXml);
	...
	protected List<Element> setAppEntityList(Document outNoStdXml) throws Exception {
		JdomUtil.print(outNoStdXml);
		Element appEntityEle = (Element) XPath.selectSingleNode(outNoStdXml, "//TranDatas");
		List<Element> list = new ArrayList<Element>();
		if(appEntityEle != null){
...
//		strBuff.append("100109").append(DateUtil.getCur8Date()).append(".xml");
		strBuff.append("100109").append(tranDate).append(".xml");
