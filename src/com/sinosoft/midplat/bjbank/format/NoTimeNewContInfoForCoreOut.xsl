...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/">
	   <TranData>
	      <AllNum><xsl:value-of select="TranData/Body/Count"/></AllNum>
		  <AllMoney><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fenToYuan(sum(TranData/Body/Detail/ActSumPrem))"/></AllMoney>
	      <PolCount></PolCount>
	      <FileName></FileName>
	      <TranDatas>
	...
	<xsl:template match="Detail">
	   <xsl:variable name="MainRisk" select="Risk[RiskCode=MainRiskCode]"/>
		<TranData>
		    <BaseInfo>
			  <BankDate><xsl:value-of select="java:com.sinosoft.midplat.common.DateUtil.date8to10($MainRisk/SignDate)"/></BankDate>
			  <BankCode>01</BankCode>
			  <!--<ZoneNo><xsl:value-of  select="substring(NodeNo,1,5)"/></ZoneNo>-->
			  <!--<BrNo><xsl:value-of  select="substring(NodeNo,6)"/></BrNo>-->
			  <ZoneNo></ZoneNo>
			  <BrNo></BrNo>
			  <TellerNo><xsl:value-of select="TellerNo"/></TellerNo>
			  <TransrNo><xsl:value-of select="TranNo"/></TransrNo>
			  <FunctionFlag>14</FunctionFlag>
			  <InsuID>10</InsuID>
		   </BaseInfo>
		   <RetData>
			  <Flag>1</Flag>
		   ...
		   <LCCont>
		      ...
		      <LCAppnt>	
			     ...
			     <AppntBirthday><xsl:value-of select="java:com.sinosoft.midplat.common.DateUtil.date8to10(Appnt/Birthday)"/></AppntBirthday>			<!--—投保人出生日期-->
			     <AppntIDType>
		    ...
		    <LCInsureds> <!-- 被保人列表 -->
			   ...
			   <LCInsured>	<!-- 被保人信息 -->
				   ...
				   <Birthday><xsl:value-of select="java:com.sinosoft.midplat.common.DateUtil.date8to10(Insured/Birthday)"/></Birthday>						<!-- 被保人出生日期 -->
				   <IDType>
					  <xsl:call-template name="tran_idtype">
						 <xsl:with-param name="idtype">
