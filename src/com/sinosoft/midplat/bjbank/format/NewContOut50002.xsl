	...
	<xsl:template name="LCCont" match="Body">
		...
		<Risks> <!-- 险种列表 -->
			...
			<xsl:for-each select="Risk[RiskCode=MainRiskCode]">
				<Risk> <!-- 险种 -->
					...
					<UpperPrem><xsl:value-of select ="$UpperPrem"/></UpperPrem>            		<!-- 保险费大写 -->
	          		<Prem><xsl:value-of select ="java:com.sinosoft.midplat.common.NumberUtil.fenToYuan($Prem)"/></Prem>            						<!-- 保险费 -->
           			<PremItems>
					...
					<UpperAmnt><xsl:value-of select ="$AmntText"/></UpperAmnt>								<!-- 保额大写 -->
					<Amnt><xsl:value-of select ="java:com.sinosoft.midplat.common.NumberUtil.fenToYuan($Amnt)"/></Amnt>													<!-- 保额 -->
					<Rate></Rate>													<!-- 费率 -->
					<Rank></Rank>            						<!-- 档次 -->						
					<Mult><xsl:value-of select ="$RiskMult"/></Mult>            						<!-- 投保份数 -->	
					<PayDateChn>
					...
					<InsuYearFlag>A</InsuYearFlag>					<!-- 保险年龄年期标志 -->
					<InsuYear>106</InsuYear>									<!-- 保险年龄年期 -->	
					<CValiDate>
		...
		<Print>
			<Print1>
				...
				<Page1Print/>
				<Page1Print/>
				<Page1Print><xsl:text>　　　保险单号码：</xsl:text><xsl:value-of select="ContNo" /><xsl:text>-(00001～</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(concat($ContPlanMultWith0,')'),5)" />

				<xsl:text>                                      币值单位：人民币元</xsl:text></Page1Print>
				...
				<Page1Print>　　　------------------------------------------------------------------------------------------------</Page1Print>
				<Page1Print/>
				<Page1Print>　　　保险费合计：<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fenToYuan(ActSumPrem)"/>元（小写）</Page1Print>
				
				<Page1Print>　　　------------------------------------------------------------------------------------------------</Page1Print>
				<Page1Print/>
				<Page1Print>　　　------------------------------------------------------------------------------------------------</Page1Print>
				<xsl:variable name="SpecContent" select="SpecContent"/>
				<Page1Print>　　　保险单特别约定：</Page1Print>
					<xsl:choose>
						<xsl:when test="$SpecContent=''">
							<Page1Print><xsl:text>（无）</xsl:text></Page1Print>
						</xsl:when>
						<xsl:otherwise>
							<Page1Print>　　　    您购买的保险产品为《安邦长寿稳赢保险计划》。若您在保险合同生效两年内退保，本公司仅退还主险的</Page1Print>
							<Page1Print>　　　现金价值。</Page1Print>
							<Page1Print>　　　    当安邦长寿稳赢1号两全保险到达合同约定的满期日时，该主险合同效力终止，附加险合同同时终止。不</Page1Print>
							<Page1Print>　　　低于主险合同及附加险合同项下满期保险金之和的101.5%的金额将自动转入安邦长寿添利终身寿险（万能型）</Page1Print>
							<Page1Print>　　　的个人账户中，该万能险合同自上述款项自动转入次日零时起生效。</Page1Print>
						</xsl:otherwise>
					</xsl:choose>
				<Page1Print/>
				<Page1Print>　　　-------------------------------------------------------------------------------------------------</Page1Print>
			...
			<xsl:variable name="printmult1" select="concat('-(00001～',$ContPlanMultWith0)"/>
 	        <xsl:variable name="printmult2" select="concat($printmult1,')')"/>
				...
				<Page2Print/>
				<Page2Print>　　　保险单号码：<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(concat(ContNo,$printmult2), 60)"/>币值单位：人民币元 </Page2Print>
				<Page2Print>　　　------------------------------------------------------------------------------------------------</Page2Print>
		        ...
		        <Page2Print><xsl:text/>　　　保单年度末<xsl:text>                 </xsl:text>
				   <xsl:text>现金价值                                现金价值</xsl:text></Page2Print>
		   	       <xsl:variable name="EndYear" select="EndYear"/>
		        <xsl:for-each select="$MainRisk/CashValues/CashValue">
			    <xsl:variable name="EndYear" select="EndYear"/>
			    <Page2Print>
