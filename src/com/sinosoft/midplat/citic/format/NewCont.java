...
public class NewCont extends XmlSimpFormat {
	...
	public Document noStd2Std(Document pNoStdXml) throws Exception {
        ...
        if("50002".equals(mainRiskCode)){// 走产品组合
			...
			cLogger.info("CITIC_中信银行，进入NewContInXsl50002进行报文转换，产品组合编码contPlanCode=[" + mainRiskCode + "]");
		}
        //add by duanjz 2015-6-17 增加安享5号组合产品50012、安享3号组合产品50011   begin
        else if("50012".equals(mainRiskCode) || "50011".equals(mainRiskCode)){// 走产品组合
			// 50012-安邦长寿安享5号保险计划: L12070-安邦长寿安享5号年金保险、L12071-安邦附加长寿添利5号两全保险（万能型）组成
        	// 50011-安邦长寿安享3号保险计划: L12068-安邦长寿安享3号年金保险、L12069-安邦附加长寿添利3号两全保险（万能型）组成
			mStdXml = NewContInXsl50012.newInstance().getCache().transform(pNoStdXml);
			cLogger.info("CITIC_中信银行，进入NewContInXsl50012进行报文转换，产品组合编码contPlanCode=[" + mainRiskCode + "]");
		}
        //add by duanjz 2015-6-17 增加安享5号组合产品50012    end
        else{	// 走非产品组合
			
			mStdXml = NewContInXsl.newInstance().getCache().transform(pNoStdXml);
			cLogger.info("CITIC_中信银行，NewContInXsl进行报文转换，主险riskCode=[" + mainRiskCode + "]");
		}       
        //add 20160302 PBKINSR-1106 中信浙江分行专属产品需求 begin
        if("122010".equals(mainRiskCode) && "0108".equals(bkBrchNo.substring(0,4))){//满足该条件为中信浙江分行专属产品
        	...
        	riskCode.setText("L12090");//浙江分行专属产品盛世3号
        	mainRiskCodeIn.setText("L12090");//浙江分行专属产品盛世3号
        }  
        //add 20160302 PBKINSR-1106 中信浙江分行专属产品需求 begin	
        //add 20160426 PBKINSR-1273  中信济南分行专属产品需求 begin
	...
	public Document std2NoStd(Document pStdXml) throws Exception {
        ...
        if("50015".equals(contPlanCode)){	// 走产品组合
        	// 长寿稳赢套餐，产品代码由50002升级为50015
        	// 50002-安邦长寿稳赢2号两全保险组合: 122046-安邦长寿稳赢1号两全保险、122047-安邦附加长寿稳赢两全保险、122048-安邦长寿添利终身寿险（万能型）组成
        	// 50015-安邦长寿稳赢2号两全保险组合: 122046-安邦长寿稳赢1号两全保险、122047-安邦附加长寿稳赢两全保险、L12081-安邦长寿添利终身寿险（万能型）组成
        	cLogger.info("CITIC_中信银行，进入NewContOutXsl50002进行报文转换，产品组合编码contPlanCode=[" + contPlanCode + "]");
        	mNoStdXml = NewContOutXsl50002.newInstance().getCache().transform(pStdXml);
        }
        //add by duanjz 2015-6-17  增加安邦长寿安享5号保险计划50012、安享3号50011 begin
        else if("50012".equals(contPlanCode) || "50011".equals(contPlanCode)){	// 走产品组合
        	// 50012-安邦长寿安享5号保险计划: L12070-安邦长寿安享5号年金保险、L12071-安邦附加长寿添利5号两全保险（万能型）组成
        	// 50011-安邦长寿安享3号保险计划: L12068-安邦长寿安享3号年金保险、L12069-安邦附加长寿添利3号两全保险（万能型）组成
        	cLogger.info("CITIC_中信银行，进入NewContOutXsl50012进行报文转换，产品组合编码contPlanCode=[" + contPlanCode + "]");
        	mNoStdXml = NewContOutXsl50012.newInstance().getCache().transform(pStdXml);
        }
        //add by duanjz 2015-6-17  增加安邦长寿安享5号保险计划50012、安享3号50011 end
        else{
        	cLogger.info("CITIC_中信银行，NewContOutXsl进行报文转换");
