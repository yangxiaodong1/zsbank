...
<Detail_List>
	...
	<Detail>
		...
		<BkDetail1></BkDetail1>
		<BkDetail1></BkDetail1>
		<BkDetail1><xsl:text>　　　　　保险单号码：</xsl:text><xsl:value-of select="ContNo" /><xsl:text>-(00001～</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(concat($ContPlanMultWith0,')'),5)" /><xsl:text>                                     币值单位：人民币元</xsl:text></BkDetail1>
		<BkDetail1>　　　　　------------------------------------------------------------------------------------------------</BkDetail1>
		<BkDetail1>
		...
		<xsl:choose>
			...
			<xsl:when test="$SpecContent!=''">
				<BkDetail1><xsl:text>　　　　　    您购买的保险产品为《安邦长寿稳赢保险计划》。若您在保险合同生效两年内退保，本公司仅退还主险的</xsl:text></BkDetail1>
				<BkDetail1><xsl:text>　　　　　现金价值。</xsl:text></BkDetail1>
				<BkDetail1><xsl:text>　　　　　    当安邦长寿稳赢1号两全保险到达合同约定的满期日时，该主险合同效力终止，附加险合同同时终止。不</xsl:text></BkDetail1>
				<BkDetail1><xsl:text>　　　　　低于主险合同及附加险合同项下满期保险金之和的101.5%的金额将自动转入安邦长寿添利终身寿险（万能型）</xsl:text></BkDetail1>
				<BkDetail1><xsl:text>　　　　　的个人账户中，该万能险合同自上述款项自动转入次日零时起生效。</xsl:text></BkDetail1>
			</xsl:when>
		</xsl:choose>
		<xsl:choose>
...
<xsl:if test="$MainRisk/CashValues/CashValue != ''">
 <Detail_List>
	...
	<Detail>
		...
		<BkDetail1></BkDetail1>
		<BkDetail1>　　　　　保险单号码：<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(concat(ContNo,$printmult2), 60)"/>币值单位：人民币元 </BkDetail1>
		<BkDetail1>　　　　　------------------------------------------------------------------------------------------------</BkDetail1>
	...
	<xsl:if test="$RiskCount=1">
	 ...
	 <xsl:if test="$RiskCount!=1">
		...
		<BkDetail1><xsl:text/>　　　　　保单年度末<xsl:text>                    现金价值                                </xsl:text>
				   <xsl:text>现金价值</xsl:text></BkDetail1>
		   	<xsl:variable name="EndYear" select="EndYear"/>
		   <xsl:for-each select="$MainRisk/CashValues/CashValue">
			 <xsl:variable name="EndYear" select="EndYear"/>
			   <BkDetail1>
					 ...
					 <xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(EndYear,30)"/>
