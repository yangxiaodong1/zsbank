			...
			<OLifeExtension>
				...
				<ContPrint>
					...
					<xsl:choose>
						...
						<xsl:when test="$SpecContent='' and $AppIDType!='6' and $AppIDType!='7' and $InsIDType!='6' and $InsIDType!='7'">
							<PrintInfo><xsl:text>（无）</xsl:text></PrintInfo>
						</xsl:when>
						<xsl:when test="$SpecContent!=''">
							<PrintInfo>
								<xsl:text>　　　    《安邦长寿安享3号年金保险计划》由主险《安邦长寿安享3号年金保险》及附加险《安邦附加长寿添</xsl:text>
							</PrintInfo>
							<PrintInfo>
								<xsl:text>　　　利3号两全保险（万能型）》组成。经生存年金受益人同意后，主险产品的生存年金，在年金给付日自动</xsl:text>
							</PrintInfo>
							<PrintInfo>
								<xsl:text>　　　转入附加险万能账户中，享受日日复利，月月结息的资金增值服务。</xsl:text>
							</PrintInfo>
						</xsl:when>
					</xsl:choose>
					<xsl:choose>
					<xsl:when test="$AppIDType='6' or $AppIDType='7' or $InsIDType='6' or $InsIDType='7'">
						<PrintInfo>
							<xsl:text>　　　    本合同中提到的‘港澳回乡证’为‘港澳居民来往大陆通行证’简称，本合同中提到的‘台胞证’为‘</xsl:text>									
						</PrintInfo>
						<PrintInfo>
							<xsl:text>　　　台湾居民来往大陆通行证’简称。</xsl:text>
						</PrintInfo>
					</xsl:when>
					<xsl:otherwise>
						<PrintInfo/>
				...
				<CashValue>
					...
					<xsl:if test="$MainRisk/CashValues/CashValue != ''">
						...
						<PrintInfo>　　　------------------------------------------------------------------------------------------------</PrintInfo>
						<PrintInfo>　　　被保险人姓名：<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(Insured/Name, 49)"/><xsl:text>险种名称：</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_($MainRisk/RiskName,0)"/></PrintInfo>
                        ...
                        <xsl:if test='position() &lt; $n'>
                            ...
                            <xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(concat(java:com.sinosoft.midplat.common.NumberUtil.fenToYuan(../CashValue[$n2]/Cash), '元'),0)"/></xsl:if></PrintInfo>
                       </xsl:if>
				       </xsl:for-each>		            
						<PrintInfo>　　　------------------------------------------------------------------------------------------------</PrintInfo>
						<PrintInfo/>
