package com.sinosoft.midplat.hbyh.net;
...
public class HbyhNetImpl extends SocketNetImpl {

	// 银行固定标识
	private static final String BANK_FLAG = "HEBB";
	// 编码集
	private static final String GBK_ENCODING = "GBK";
	// 银行交易代码
	private String cOutFuncFlag = null;
	
	public HbyhNetImpl(Socket socket, Element thisConfRoot) throws MidplatException {
	...
	public Document receive() throws Exception {
		...
		cLogger.info("Into HbyhNetImpl.receive()...");
		
		/*
		 * 每个数据包的基本格式为：包头+包长+包体。
		 * 包头：HEBB（4字符）+交易码（4字符）。其中HEBB为标识码，包头总占位为8个字符。
...
//		System.out.println(new String (mBodyBytes));
	...
	public void send(Document pOutNoStd) throws Exception {
		...
		cLogger.info("Into HbyhNetImpl.send()...");
		...
		cLogger.info("Out HbyhNetImpl.send()!");
	}
