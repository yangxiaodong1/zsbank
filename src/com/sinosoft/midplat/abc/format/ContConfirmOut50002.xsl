	  ...
	  <RetData>
		...
		<xsl:if test="Head/Flag='0'">
			...
			<Base>
				...
				<RiskName><xsl:value-of select ="Body/ContPlan/ContPlanName"/></RiskName>
			...
			<Risks>
				...
				<xsl:for-each select="Body/Risk">
					...
					<Risk>
						...
						<Prem><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fenToYuan(ActPrem)"/></Prem>
        ...
        <Prnts>
			...
			<Prnt>
				<Value><xsl:text/>保险费合计：<xsl:value-of select="Body/ActSumPremText"/>（RMB<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fenToYuan(Body/ActSumPrem)"/>元整）</Value>
			</Prnt>
			<Prnt>
            	<Value>------------------------------------------------------------------------------------------------</Value>
		    </Prnt>
			...
			<Prnt>
   		    ...
   		    <xsl:choose>
   		    	<xsl:when test="Body/SpecContent = ''">
   		    		<Prnt>
   		    			<Value>
   		    				<xsl:text>（无）</xsl:text>
   		    			</Value>
   		    	...
   		    	<xsl:otherwise>
   		    		<Prnt>
   		    			<Value>
   		    				<xsl:text>    您购买的保险产品为《安邦长寿稳赢保险计划》。安邦附加长寿稳赢两全保险为本公司赠送保险，不向您收取</xsl:text>
   		    			</Value>
   		    		</Prnt>
   		    		<Prnt>
   		    			<Value>
   		    				<xsl:text>保险费。若您在该附加险合同生效后两年内退保，本公司不退还该附加险合同项下的现金价值。</xsl:text>
   		    			</Value>
					</Prnt>
					<Prnt>
   		    			<Value>
   		    				<xsl:text>    当安邦长寿稳赢1号两全保险到达合同约定的满期日时，该主险合同效力终止，附加险合同同时终止。不低于</xsl:text>
   		    			</Value>
   		    		</Prnt>
					<Prnt>
   		    			<Value>
   		    				<xsl:text>主险合同及附加险合同项下满期保险金之和的104%的金额将自动转入安邦长寿添利终身寿险（万能型）的个人</xsl:text>
   		    			</Value>
   		    		</Prnt>
   		    		<Prnt>
   		    			<Value>
   		    				<xsl:text>账户中，该万能险合同自上述款项自动转入次日零时起生效。</xsl:text>
   		    			</Value>
   		    		</Prnt>
   		    	</xsl:otherwise>
   		    </xsl:choose>
...
<Messages>
    ...
    <xsl:if test="$RiskCount!=1">
    	...
    	<Message>
    		<Value><xsl:text>保单年度末                      现金价值                                现金价值</xsl:text></Value>
    	</Message>
    	<xsl:for-each select="$MainRisk/CashValues/CashValue">
