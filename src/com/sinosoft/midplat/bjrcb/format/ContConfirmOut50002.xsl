...
<xsl:stylesheet version="1.0"
				...
				<xsl:choose>
					...
					<xsl:otherwise>
						<Context><xsl:text>　　　</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(RiskName, 39)"/>
							...
							<xsl:choose>
								<xsl:when test="PayIntv = 0">
									<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_('一次性', 14)"/>
								</xsl:when>
								<xsl:when test="PayEndYearFlag = 'Y'">
									<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(concat(PayEndYear, '年'), 14)"/>
								</xsl:when>
	...
	<xsl:template name="tran_riskcode">
		...
		<xsl:choose>
			...
			<!-- 安邦惠农2  -->
			<xsl:when test="$riskcode='122012'">000408</xsl:when>
			<!-- 安邦盛世2号终身寿险（万能型）  -->
			<xsl:when test="$riskcode='122010'">000409</xsl:when>
			<!-- 安邦盛世3号终身寿险（万能型）  -->
			<xsl:when test="$riskcode='122009'">000410</xsl:when>
			<!-- 安邦黄金鼎5号两全保险（分红型）A款  -->
			<!-- FIXME 待银行明确后修改组合产品的riskocde -->
			
			<xsl:when test="$riskcode='122046'">000483</xsl:when>
