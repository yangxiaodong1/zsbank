...
public class IcbcEdrBlcService extends ServiceImpl {
	public IcbcEdrBlcService(Element pThisBusiConf) {
		super(pThisBusiConf);
	}
	
	public Document service(Document pInXmlDoc) {
		long mStartMillis = System.currentTimeMillis();
		cLogger.info("Into IcbcEdrBlcService.service()...");
		cInXmlDoc = pInXmlDoc;
		...
		try {
			...
			if ("1".equals(tExeSQL.getOneValue(tSqlStr))) {
				throw new MidplatException("已成功保全对账，不能重复操作！");
			...
			cOutXmlDoc = new CallWebsvcAtomSvc(AblifeCodeDef.SID_BANK_EDR_BAL_SERVICE).call(cInXmlDoc);			
