...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="Body" match="Policy">
		...
		<GetBankAccNo>
			<xsl:value-of select="../../Holding/Banking/AccountNumber" />
		</GetBankAccNo>
		<GetBankAccName>
			<xsl:value-of select="../../Holding/Banking/AcctHolderName" />
		</GetBankAccName>
		<CertifyName>
			<xsl:value-of select="../../FormInstance/FormName" />
		</CertifyName>
		<CertifyCode>
			<xsl:value-of select="../../FormInstance/ProviderFormNumber" />
		</CertifyCode>
		<RepeatType>0</RepeatType><!-- 重送标志 （0-否 1-是）-->
