...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:java="http://xml.apache.org/xslt/java" exclude-result-prefixes="java">
				...
				<Text>
					...
					<TextContent>
						<TextRowContent>
							...
							<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(Appnt/Name, 24)"/>
					...
					<xsl:choose>
						...
						<xsl:otherwise>
							...
							<TextContent>
								<TextRowContent>    当安邦长寿稳赢1号两全保险到达合同约定的满期日时，该主险合同效力终止，附加险合同同时终止。不低</TextRowContent>
							</TextContent>
							<TextContent>
								<TextRowContent>于主险合同及附加险合同项下满期保险金之和的101.5%的金额将自动转入安邦长寿添利终身寿险（万能型）的</TextRowContent>
							</TextContent>
							<TextContent>
								<TextRowContent>个人账户中，该万能险合同自上述款项自动转入次日零时起生效。</TextRowContent>
							</TextContent>
						</xsl:otherwise>
					</xsl:choose>
					<TextContent>
					...
					<TextContent>
						<TextRowContent>保险合同成立日期：<xsl:value-of  select="substring($MainRisk/SignDate,1,4)"/>年<xsl:value-of  select="substring($MainRisk/SignDate,5,2)"/>月<xsl:value-of  select="substring($MainRisk/SignDate,7,2)"/>日<xsl:text>                               </xsl:text>保险合同生效日期：<xsl:value-of  select="substring($MainRisk/CValiDate,1,4)"/>年<xsl:value-of  select="substring($MainRisk/CValiDate,5,2)"/>月<xsl:value-of  select="substring($MainRisk/CValiDate,7,2)"/>日</TextRowContent>
					</TextContent>
					<TextContent>
						<TextRowContent>保险销售人员：<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(AgentName,49)"/>从业资格证编号：<xsl:value-of select="AgentCertiCode"/></TextRowContent>
					</TextContent>
					<TextContent>
						<TextRowContent/>
					</TextContent>
					...
					<TextContent>
						<TextRowContent>代理机构名称：<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(AgentComName, 49)"/>业务许可证编号：<xsl:value-of select="AgentComCertiCode"/></TextRowContent>
					</TextContent>
					<TextContent>
						<TextRowContent>银行保险业务负责人：<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(AgentManageName, 43)"/>工号：<xsl:value-of select="AgentManageNo"/></TextRowContent>
					</TextContent>
					<TextContent>
						<TextRowContent>银行销售人员：<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(TellerName,49)"/>从业资格证编号：<xsl:value-of select="TellerCertiCode"/></TextRowContent>
					</TextContent>
				</Text>
			</SubVoucher>
	...
	<xsl:template name="Coverage" match="Risk">
		...
		<Coverage>
			...
			<xsl:choose>
				<!-- FIXME 这部分与就接口文档里的描述正好相反，接口文档中1=终身，9=非终身 -->
	...
	<xsl:template name="tran_ProductCode" match="RiskCode">
		<xsl:choose>
			...
			<xsl:when test=".=122046">013</xsl:when>	<!-- 组合产品-50002: 122046-安邦长寿稳赢1号两全保险、122047-安邦附加长寿稳赢两全保险、122048-安邦长寿添利终身寿险（万能型）组成  -->
			<xsl:otherwise>--</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	<!-- 缴费形式
