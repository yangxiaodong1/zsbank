...
public class IcbcBDZYFrozenFormat extends XmlSimpFormat {
	

	public IcbcBDZYFrozenFormat(Element pThisBusiConf) {
		super(pThisBusiConf);
	}

	public Document noStd2Std(Document pNoStdXml) throws Exception {
		cLogger.info("Into IcbcBDZYFrozenFormat.noStd2Std()...");


		Document mStdXml = 
			IcbcBDZYFrozenFormatInXsl.newInstance().getCache().transform(pNoStdXml);
...
//根据质押贷款查询交易，获取银行账户，账户名以供核心使用 
		...
		cLogger.info("Out IcbcBDZYFrozenFormat.noStd2Std()!");
		return mStdXml;
	}
	
	public Document std2NoStd(Document pStdXml) throws Exception {
		cLogger.info("Into IcbcBDZYFrozenFormat.std2NoStd()...");
		
		Document mNoStdXml = 
			IcbcBDZYFrozenFormatOutXsl.newInstance().getCache().transform(pStdXml);
		
		
		
		cLogger.info("Out IcbcBDZYFrozenFormat.std2NoStd()!");
		return mNoStdXml;
	}
}
