...
/**
 * @Title: com.sinosoft.midplat.icbc.format.IcbcSelfTermTBEdrFormat.java
 * @Description: 工行自助终端犹豫期退保
 * Copyright: Copyright (c) 2014
 * Company:安邦保险IT部
 * 
 * @date Jul 21, 2014 4:14:11 PM
 * @version 
 *
 */
public class IcbcSelfTermTBEdrFormat extends XmlSimpFormat{

	public IcbcSelfTermTBEdrFormat(Element pThisBusiConf) {
		super(pThisBusiConf);
	}

	public Document noStd2Std(Document pNoStdXml) throws Exception {
		cLogger.info("Into IcbcSelfTermTBEdrFormat.noStd2Std()...");

		Document mStdXml = IcbcSelfTermTBEdrFormatInXsl.newInstance().getCache().transform(pNoStdXml);
		/**
		...
		if("0".equals(IcbcUtil.zjCheck(regionCode, teller))){
			...
			if(certifyCode == null || certifyCode.trim().equals("")){
				...
				//自助终端犹退查询交易
				tSqlStr.append(" and FuncFlag=142");
		...
		cLogger.info("Out IcbcSelfTermTBEdrFormat.noStd2Std()!");
		return mStdXml;
	}

	public Document std2NoStd(Document pStdXml) throws Exception {
		
		cLogger.info("Into IcbcSelfTermTBEdrFormat.std2NoStd()...");
		...
		if("50002".equals(tMainRiskCode) || "50015".equals(tMainRiskCode)){
			 ...
			 */
			mNoStdXml = IcbcSelfTermTBEdrFormatOutXsl50002.newInstance().getCache().transform(pStdXml);
			
			cLogger.info("ICBC_工商银行，进入IcbcSelfTermTBEdrFormatOutXsl50002进行报文转换。");
			
		}else{	// 非组合产品
			mNoStdXml = IcbcSelfTermTBEdrFormatOutXsl.newInstance().getCache().transform(pStdXml);
			
			cLogger.info("ICBC_工商银行，进入IcbcSelfTermTBEdrFormatOutXsl进行报文转换。");
		}
		
		cLogger.info("Out IcbcSelfTermTBEdrFormat.std2NoStd()!");
		return mNoStdXml;
	}
...
//		out.write(JdomUtil.toStringFmt(new IcbcSelfTermTBEdrFormat(null).noStd2Std(doc)));
		...
		out.write(JdomUtil.toStringFmt(new IcbcSelfTermTBEdrFormat(null).std2NoStd(doc)));
