...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:java="http://xml.apache.org/xslt/java" exclude-result-prefixes="java">
		...
		<OLifE>
			<Holding id="cont">
				...
				<Policy>
					...
					<EffDate><xsl:value-of select="java:com.sinosoft.midplat.common.DateUtil.date8to10($MainRisk/CValiDate)"/></EffDate>
					<Life>
					...
					<OLifEExtension>
						...
						<CalAmount/>
						<!-- 试算后总保费 -->
						<WithdrawalDescription>您的犹豫期为银行系统承保日期次日起的15日，如果您在此期间内申请解约，保险公司将全额无息退还已收取的保险费；如果您在此之后申请解约，保险公司将按合同条款约定支付退保金。由于各地监管有特殊要求，对于犹豫期的计算若存在特殊情况，以各地监管要求为准</WithdrawalDescription>
						<!-- 犹豫期退保说明 -->
					</OLifEExtension>
				</Policy>
			</Holding>
		...
		<Print>
			...
			<SubVoucher>
				...
				<Text>
					...
					<TextContent>
						<TextRowContent>
							<xsl:text/>保险单号码：<xsl:value-of select="ContNo"/><xsl:text>-(00001～</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(concat($ContPlanMultWith0,')'),5)" />                                        币值单位：人民币元</TextRowContent>
					...
					<TextContent>
						<TextRowContent>银行网点名称：<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(AgentComName, 49)"/>业务许可证编号：<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(AgentComCertiCode, 20)"/></TextRowContent>
					</TextContent>
					<TextContent>
						<TextRowContent>银行销售人员：<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(TellerName, 49)"/>从业资格证编号：<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(TellerCertiCode, 20)"/></TextRowContent>
					</TextContent>
					<!-- 
					<RowTotal><xsl:value-of select="43 + $Ser1 + $RiskCount"/></RowTotal>
					 -->
				</Text>
			</SubVoucher>
 	        ...
 	        <xsl:variable name="printmult2" select="concat($printmult1,')')"/>
				<SubVoucher>
					...
					<Text>
						...
						<TextContent>
							<TextRowContent>
							<xsl:text/>保险单号码：<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(concat(ContNo,$printmult2), 60)"/>币值单位：人民币元 </TextRowContent>
						</TextContent>
						<TextContent>
							<TextRowContent>
							<xsl:text/>------------------------------------------------------------------------------------------------</TextRowContent>
		...
		<Coverage>
			...
			<OLifEExtension>
				<xsl:choose>
					...
					<xsl:when test="(PayEndYear = 1000) and (PayEndYearFlag = 'Y')">
	...
	<xsl:template name="tran_ProductCode" match="RiskCode">
		<xsl:choose>
			...
			<xsl:when test=".=L12081">013</xsl:when>	<!-- 前五年50002这款产品的主险为122046，五年后这款主险的产品为L12081,如果5年后做退保时核心传的主险险种便成了L12081 -->
			<!-- PBKINSR-679 工行盛2（柜面、网银、自助终端）、盛3（柜面、网银、自助终端）、盛9（柜面）、50002（柜面、网银、自助终端）产品升级 -->
