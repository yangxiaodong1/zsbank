	...
	<xsl:template name="Body" match="Body">
		<Body>
			...
			<PRINT>
				...
				<PRINT_LINE>　保险单特别约定：</PRINT_LINE>
					<xsl:choose>
						...
						<xsl:otherwise>
							...
							<PRINT_LINE>　低于主险合同及附加险合同项下满期保险金之和的104%的金额将自动转入安邦长寿添利终身寿险（万能型）的</PRINT_LINE>
							<PRINT_LINE>　个人账户中，该万能险合同自上述款项自动转入次日零时起生效。</PRINT_LINE>
						</xsl:otherwise>
					</xsl:choose>
				...
				<PRINT_LINE><xsl:text>　银行销售人员：</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(TellerName, 49)"/><xsl:text>银行销售人工号：</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(SellerNo, 20)"/></PRINT_LINE>
