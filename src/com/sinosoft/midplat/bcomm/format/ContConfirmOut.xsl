...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="tran_RiskCode" match="RiskCode">
		<xsl:choose>
			...
			<xsl:when test=".='122048'">122048</xsl:when><!-- 安邦长寿添利终身寿险（万能型） -->
			<!-- add 20150819 增加安享5号产品  begin -->
			<xsl:when test=".='50012'">50012</xsl:when><!-- 安邦长寿安享5号保险计划 -->
			<xsl:when test=".='L12070'">50012</xsl:when><!-- 安邦长寿安享5号年金保险 -->
			<xsl:when test=".='L12071'">L12071</xsl:when><!-- 安邦附加长寿添利5号两全保险（万能型） -->
			<!-- add 20150819 增加安享5号产品  end -->
