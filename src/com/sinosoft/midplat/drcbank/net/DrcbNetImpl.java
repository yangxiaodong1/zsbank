package com.sinosoft.midplat.drcbank.net;
...
public class DrcbNetImpl extends SocketNetImpl{

	// 银行固定标识
	private static final String BANK_FLAG = "DRCB";
	// 编码集
	private static final String GBK_ENCODING = "GBK";
	// 银行交易代码
	private String cOutFuncFlag = null;
	
	public DrcbNetImpl(Socket socket, Element thisConfRoot) throws MidplatException {
	...
	public Document receive() throws Exception {
		...
		cLogger.info("Into DrcbNetImpl.receive()...");
		
		/*
		 * 每个数据包的基本格式为：包头+包长+包体。
		 * 包头：DRCB（4字节）+交易码（4字节）。其中DRCB为标识码，包头总占位为8个字节。。
...
//		cLogger.debug("原始报文[" + new String(mBodyBytes, GBK_ENCODING) + "]");
	...
	public void send(Document pOutNoStd) throws Exception {
		...
		cLogger.info("Into DrcbNetImpl.send()...");
		...
		cLogger.info("Out DrcbNetImpl.send()!");
	}
