 ...
 <TranData>
	...
	<Body>
		...
		<PubContInfo>
			...
			<BankName></BankName>
			<!-- 补退费帐户帐号--><!-- 确认交易时为必填项 -->
			<BankAccNo><xsl:value-of select="accountnumber" /></BankAccNo>
			<!-- 补退费帐户户名--><!-- 确认交易时为必填项 -->
			<BankAccName><xsl:value-of select="fullname" /></BankAccName>
			<!--领款人证件类型 -->
		...
		<EdorPNInfo>
			...
			<LoanMoney><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.yuanToFen(amount)" /></LoanMoney>
		</EdorPNInfo>
	</Body>
</TranData>
</xsl:template>
<!-- 险种代码-->
	<xsl:template match="productcode">
		<xsl:choose>
			<xsl:when test=".='008'">L12077</xsl:when>	<!-- 安邦盛世2号终身寿险（万能型），该产品代码只针对浙江工行的盛2  -->
