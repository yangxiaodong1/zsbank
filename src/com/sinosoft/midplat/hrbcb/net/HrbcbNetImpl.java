package com.sinosoft.midplat.hrbcb.net;
...
public class HrbcbNetImpl extends SocketNetImpl {

	// 银行固定标识
	private static final String BANK_FLAG = "HRBB";
	// 编码集
	private static final String GBK_ENCODING = "GBK";
	// 银行交易代码
	private String cOutFuncFlag = null;
	
	public HrbcbNetImpl(Socket socket, Element thisConfRoot) throws MidplatException {
	...
	public Document receive() throws Exception {
		...
		cLogger.info("Into HrbcbNetImpl.receive()...");
		
		/*
		 * 每个数据包的基本格式为：包头+包长+包体。
		 * 包头：HRBB（4字符）+交易码（6字符）。其中JLYH为标识码，包头总占位为8个字符。
		...
		cLogger.debug("请求报文头[" + xmlHead + "]");
		cOutFuncFlag =  xmlHead.substring(4, 10);
		int mBodyLen = Integer.parseInt(xmlHead.substring(10).trim());
		...
		cLogger.debug("原始报文[" + new String(mBodyBytes, GBK_ENCODING) + "]");
	...
	public void send(Document pOutNoStd) throws Exception {
		...
		cLogger.info("Into HrbcbNetImpl.send()...");
		...
		cLogger.info("Out HrbcbNetImpl.send()!");
	}


	public static void main(String[] args){
		String str = "SRBC1612A009876543";
