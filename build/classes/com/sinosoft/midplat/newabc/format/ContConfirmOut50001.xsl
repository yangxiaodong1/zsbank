		...
		<App>
			...
			<Ret>
        		...
        		<Prnts>
					...
					<Prnt/>
					<Prnt/>
					<Prnt><xsl:text>　　　 保险单号码：</xsl:text><xsl:value-of select="ContNo" /><xsl:text>-(00001～</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(concat($ContPlanMultWith0,')'),5)" />
					<xsl:text>                                     币值单位：人民币元</xsl:text></Prnt>
					<Prnt>　　　------------------------------------------------------------------------------------------------</Prnt>
					<Prnt>
					...
					<Prnt>　　　保险单特别约定：</Prnt>
   		    		...
   		    		<Prnt>
   		    	    	<xsl:text>　　　    您购买的保险产品为《安邦长寿稳赢1号两全保险组合》。若您在保险合同生效两年内退保，本公司仅退</xsl:text>
   		    		</Prnt>
   		    		<Prnt>
   		    	    	<xsl:text>　　　还主险的现金价值。</xsl:text>
   		    		</Prnt>
					<Prnt>　　　-------------------------------------------------------------------------------------------------</Prnt>
				...
				<Messages>
					...
					<xsl:if test="$MainRisk/CashValues/CashValue != ''"> 
						...
						<Prnt/>
						<Prnt>　　　保险单号码：<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(concat(ContNo,$printmult2), 60)"/>币值单位：人民币元 </Prnt>
						<Prnt>　　　------------------------------------------------------------------------------------------------</Prnt>
				        ...
				        <Prnt><xsl:text/>　　　保单年度末                      现金价值                                现金价值</Prnt>
