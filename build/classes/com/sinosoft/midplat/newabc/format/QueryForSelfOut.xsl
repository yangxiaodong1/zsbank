...
<xsl:stylesheet version="1.0"
			...
			<Ret>
				...
				<InsuDueDate>
					...
					<xsl:choose>
						...
						<xsl:otherwise><xsl:value-of select="$MainRisk/Years" />年</xsl:otherwise>
					</xsl:choose>
				</InsuDueDate>
				<!-- 缴费方式 -->
				<PayType>
	...
	<xsl:template match="ContState">
		<xsl:choose>
			...
			<xsl:when test=".='B'">00</xsl:when><!-- 待签单-->
	...
	<xsl:template match="Nationality">
		<xsl:choose>
			<xsl:when test=".='CHN'">156</xsl:when><!-- 中國 -->
			<xsl:when test=".='JP'">392</xsl:when><!-- 日本 -->
			<xsl:when test=".='US'">840</xsl:when><!-- 美国 -->
			<xsl:when test=".='RU'">643</xsl:when><!-- 俄罗斯 -->
			<xsl:when test=".='GB'">826</xsl:when><!-- 英国 -->
