...
<Head>
	...
	<SourceType>17</SourceType><!-- 手机银行渠道 -->
</Head>
</xsl:template>

<xsl:template name="Body" match="Transaction_Body">
<IsPremCal>
  <xsl:if test="PbRemark6 = 'FALSE'">1</xsl:if>
</IsPremCal>
...
<!-- 江浙专属产品险种 -->
<xsl:variable name="ZJMainRiskCode">
	<xsl:call-template name="tran_riskcode_zj">
		<xsl:with-param name="riskcode" select="PbInsuType" />
	</xsl:call-template>
</xsl:variable>
...
<Risk>	<!-- 主险 -->
	...
	<xsl:choose>
		<xsl:when test="substring($BkBrchNo,1,4) = '0108' and $ZJMainRiskCode != ''"><!-- 浙江专属产品 -->
			<RiskCode><xsl:value-of select="$ZJMainRiskCode" /></RiskCode>
			<MainRiskCode><xsl:value-of select="$ZJMainRiskCode" /></MainRiskCode>
		</xsl:when>
		<xsl:otherwise>	<!-- 其他 -->
			<xsl:if test="$tContPlanCode = '50015'">
		        <RiskCode><xsl:value-of select="$tContPlanCode" /></RiskCode>
			    <MainRiskCode><xsl:value-of select="$tContPlanCode" /></MainRiskCode>
		    </xsl:if>
		    <xsl:if test="$tContPlanCode != '50015'">
	...
	<xsl:if test="$tContPlanCode = '50015'">
	   <!--  组合产品50015的保险年期为：终身，但是核心端以主险122046的保险期限为校验，因此要转换为5年 -->
	   <InsuYearFlag>
		  <xsl:choose>
	   ...
	   <InsuYear>
	      <!-- 组合产品50002的保险年期为：终身，但是核心端以主险122046的保险期限为校验，因此要转换为5年-->
		  <xsl:if test="$PbInsuYearFlag=1">5</xsl:if> 
...
<!-- 专属产品险种代码 -->
<xsl:template name="tran_riskcode_zj">
<xsl:param name="riskcode" />
<xsl:choose>
	<!-- PBKINSR-1278 中信银行手机银行盛世3号-浙江中信专属产品 begin -->
	<xsl:when test="$riskcode='122010'">L12090</xsl:when>	<!-- 安邦盛世3号终身寿险（万能型） -->
...
<xsl:template name="tran_ContPlanCode">
	...
	<xsl:choose>
		<!-- 长寿稳赢50015: -->
	    <xsl:when test="$contPlanCode='50002'">50015</xsl:when>	<!-- 安邦长寿稳赢保险组合计划 -->
	    <xsl:when test="$contPlanCode='122046'">122046</xsl:when>	<!-- 安邦长寿稳赢1号两全保险 -->
	    <xsl:when test="$contPlanCode='122047'">122047</xsl:when>	<!-- 安邦附加长寿稳赢两全保险 -->
	    <xsl:when test="$contPlanCode='L12081'">L12081</xsl:when>	<!-- 安邦长寿添利终身寿险（万能型） -->	   
		<xsl:otherwise></xsl:otherwise>
	</xsl:choose>
</xsl:template>
