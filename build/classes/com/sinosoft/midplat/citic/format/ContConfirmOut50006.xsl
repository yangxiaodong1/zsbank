	...
	<Detail>
		...
		<xsl:choose>
			...
			<xsl:when test="$SpecContent!=''">
				...
				<BkDetail1><xsl:text>　　　　　金价项下的现金价值。等值于主险首期保险费5%的金额将于附加险生效第一保单年度末最后一日24时注入附加</xsl:text></BkDetail1>
				<BkDetail1><xsl:text>　　　　　险万能账户。</xsl:text></BkDetail1>
			</xsl:when>
		</xsl:choose>
		<xsl:choose>
...
<xsl:template name="tran_ContPlanCode">
	...
	<xsl:choose>
		<!-- 50006-安邦长寿智赢1号年金保险计划: L12052-安邦长寿智赢1号年金保险、L12053-安邦附加长寿智赢护理保险（万能型） -->
		<xsl:when test="$contPlanCode='L12052'">50006</xsl:when>
		<xsl:otherwise>--</xsl:otherwise>	
	</xsl:choose>
</xsl:template>
