...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:java="http://xml.apache.org/xslt/java" exclude-result-prefixes="java">
							...
							<TextRowContent>
								...
								<xsl:choose>
									...
									<xsl:when test="InsuYearFlag = 'Y'">
										<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(concat(InsuYear, '年'), 10)"/>
									</xsl:when>
									<xsl:otherwise>
	...
	<xsl:template name="tran_ProductCode" match="RiskCode">
		<xsl:choose>
			...
			<xsl:when test=".='L12089'">016</xsl:when>	<!-- 安邦盛世1号终身寿险（万能型）B款  -->
			<xsl:when test=".='L12093'">017</xsl:when>	<!-- 安邦盛世9号两全保险B款（万能型）  -->
			<xsl:when test=".='L12086'">018</xsl:when>	<!-- 安邦东风3号两全保险（万能型） -->
			<xsl:when test=".='L12102'">014</xsl:when> 	<!-- 安邦盛世9号终身寿险（万能型） -->
			<xsl:when test=".='L12087'">L12087</xsl:when>	<!-- 安邦东风5号两全保险（万能型） -->
			<xsl:when test=".='L12088'">L12088</xsl:when> 	<!-- 安邦东风9号终身寿险（万能型） -->
			<xsl:when test=".='L12085'">L12085</xsl:when> 	<!-- 安邦东风2号终身寿险（万能型） -->
			<xsl:otherwise>--</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	<!-- 缴费形式
