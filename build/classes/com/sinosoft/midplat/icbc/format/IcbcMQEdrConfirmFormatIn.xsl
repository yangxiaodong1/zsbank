...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/">
		<TranData>
			...
			<Body>
				<ContNo>
					<xsl:value-of
						select="TXLife/TXLifeRequest/OLifE/Holding/Policy/PolNumber" />
				</ContNo>
				<EdorAppDate>
					<xsl:value-of
	...
	<xsl:template name="PubContInfo" match="Policy">
		<PubContInfo>
			...
			<xsl:variable name="AppntPartyNode"
				select="../../Party[@id=$AppntPartyID]" />
			...
			<!-- 投保人证件有效截止日期 -->
			<AppntIdTypeEndDate>
				<xsl:value-of select="$AppntPartyNode/GovtTermDate" />
			</AppntIdTypeEndDate>
			<!-- 投保人住宅电话 (投保人联系电话) -->
			<AppntPhone>
				<xsl:value-of select="$AppntPartyNode/Phone/DialNumber" />
			</AppntPhone>
			...
			<InsuredIDType>
				<xsl:apply-templates
					select="$InsuredPartyNode/GovtIDTC" />
			</InsuredIDType>
			<!-- 被保人证件号码 -->
			<InsuredIDNo>
			...
			<BankAccNo>
				<xsl:value-of
					select="../../../OLifE/Holding/Banking/AccountNumber" />
			</BankAccNo>
			<!-- 补退费帐户户名--><!--确认交易时为必填项-->
			<BankAccName>
				<xsl:value-of
					select="../../../OLifE/Holding/Banking/AcctHolderName" />
			</BankAccName>
		</PubContInfo>
	</xsl:template>
