...
<Detail_List>
	...
	<Detail>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<BkDetail1><xsl:text>　　　    您购买的保险产品为《安邦长寿稳赢保险计划》。安邦附加长寿稳赢两全保险为本公司赠送保险，不向您</xsl:text></BkDetail1>
				<BkDetail1><xsl:text>　　　收取保险费。若您在该附加险合同生效后两年内退保，本公司不退还该附加险合同项下的现金价值。</xsl:text></BkDetail1>
				<BkDetail1><xsl:text>　　　    当安邦长寿稳赢1号两全保险到达合同约定的满期日时，该主险合同效力终止，附加险合同同时终止。不低</xsl:text></BkDetail1>
				<BkDetail1><xsl:text>　　　于主险合同及附加险合同项下满期保险金之和的104%的金额将自动转入安邦长寿添利终身寿险（万能型）的个</xsl:text></BkDetail1>
				<BkDetail1><xsl:text>　　　人账户中，该万能险合同自上述款项自动转入次日零时起生效。</xsl:text></BkDetail1>
			</xsl:otherwise>
		</xsl:choose>
		<BkDetail1>　　　-------------------------------------------------------------------------------------------------</BkDetail1>
		<BkDetail1>
...
<xsl:if test="$MainRisk/CashValues/CashValue != ''">
 <Detail_List>
	...
	<xsl:if test="$RiskCount=1">
	 ...
	 <xsl:if test="$RiskCount!=1">
	 	<BkDetail1>
			<xsl:text>　　　险种名称：              </xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_($MainRisk/RiskName,40)"/><xsl:value-of select="Risk[RiskCode!=MainRiskCode]/RiskName"/></BkDetail1>
		<BkDetail1><xsl:text/>　　　保单年度末<xsl:text>                      </xsl:text>
				   <xsl:text>现金价值                                现金价值</xsl:text></BkDetail1>
		   	<xsl:variable name="EndYear" select="EndYear"/>
		   <xsl:for-each select="$MainRisk/CashValues/CashValue">
			 <xsl:variable name="EndYear" select="EndYear"/>
			   <BkDetail1>
...
<xsl:choose>
	<xsl:when test="$contPlanCode=122001">2001</xsl:when>	<!-- 安邦黄金鼎1号两全保险（分红型）A款 -->
	<xsl:when test="$contPlanCode=122002">2002</xsl:when>	<!-- 安邦黄金鼎2号两全保险（分红型）A款 -->
	<xsl:when test="$contPlanCode=122003">2003</xsl:when>	<!-- 安邦聚宝盆1号两全保险（分红型）A款 -->
	<xsl:when test="$contPlanCode=122004">2004</xsl:when>	<!-- 安邦附加黄金鼎2号两全保险（分红型）A款 -->
	<xsl:when test="$contPlanCode=122005">2005</xsl:when>	<!-- 安邦黄金鼎3号两全保险（分红型）A款 -->
	<xsl:when test="$contPlanCode=122006">2006</xsl:when>	<!-- 安邦聚宝盆2号两全保险（分红型）A款 -->
	<xsl:when test="$contPlanCode=122008">2008</xsl:when>	<!-- 安邦白玉樽1号终身寿险（万能型） -->
	<xsl:when test="$contPlanCode=122009">2009</xsl:when>	<!-- 安邦黄金鼎5号两全保险（分红型）A款 -->
	<xsl:when test="$contPlanCode=122010">2010</xsl:when>	<!-- 安邦盛世3号终身寿险（万能型） -->
	
	<!-- 50002: 122046-安邦长寿稳赢1号两全保险、122047-安邦附加长寿稳赢两全保险、122048-安邦长寿添利终身寿险（万能型）组成 -->
	<xsl:when test="$contPlanCode=50002">2046</xsl:when>	<!-- 安邦盛世3号终身寿险（万能型） -->
	<xsl:otherwise>--</xsl:otherwise>
</xsl:choose>
</xsl:template>
