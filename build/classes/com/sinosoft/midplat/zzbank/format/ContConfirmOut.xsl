    ...
    <xsl:template name="Transaction_Body" match="Body">
        <Body>
            ...
            <PRINT>
                ...
                <PRINT_LINE></PRINT_LINE>
                <PRINT_LINE>-------------------------------------------------------------------------------------------------</PRINT_LINE>
                <PRINT_LINE><xsl:text>保险合同成立日期：</xsl:text><xsl:value-of  select="java:com.sinosoft.midplat.common.DateUtil.formatTrans($MainRisk/PolApplyDate,'yyyyMMdd','yyyy年MM月dd日')"/><xsl:text>                               保险合同生效日期：</xsl:text><xsl:value-of  select="java:com.sinosoft.midplat.common.DateUtil.formatTrans($MainRisk/CValiDate,'yyyyMMdd','yyyy年MM月dd日')"/></PRINT_LINE>
                <PRINT_LINE></PRINT_LINE>
                <PRINT_LINE></PRINT_LINE>
                <PRINT_LINE></PRINT_LINE>
                <PRINT_LINE></PRINT_LINE>
                ...
                <PRINT_LINE></PRINT_LINE>
                <PRINT_LINE><xsl:text>为确保您的保单权益，请及时拔打我公司服务电话、登陆网站或到柜台进行查询，核实保单信息（对于保险期</xsl:text></PRINT_LINE>
                <PRINT_LINE><xsl:text>限一年期以上的寿险保单，建议您在收到保单之日起10日内完成首次查询）。</xsl:text></PRINT_LINE>
                <PRINT_LINE/>
                <PRINT_LINE><xsl:text>银行网点名称：</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(AgentComName, 45)"/><xsl:text>银行销售人员工号：</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(TellerName, 20)"/></PRINT_LINE>
            </PRINT>
