...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="Body" match="MAIN">
		...
		<!-- 争议处理方式 -->
		<DisputedFag><xsl:apply-templates select="DEALTYPE" /></DisputedFag>
		<!-- 争议仲裁机构名 -->
		<DisputedName><xsl:value-of select="INTERCED" /></DisputedName>
	...
	<xsl:template name="Appnt" match="TBR"> 
		<Appnt>
			...
			<!-- FIXME 确认银行的单位是什么? 年收入，银行单位为：元 -->
			<Salary>
				<xsl:if test="TBR_ANNUALINCOME =''"></xsl:if>
				<xsl:if test="TBR_ANNUALINCOME !=''"><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.yuanToFen(TBR_ANNUALINCOME)" /></xsl:if>
			</Salary>
			<!-- FIXME 确认银行的单位是什么? 家庭年收入，银行单位为：元 -->
			...
			<!-- 地址 -->
			<Address>
				<xsl:value-of select="substring-after(TBR_ADDR,'#')" />
			</Address>
			<!-- 邮编 -->
			<ZipCode>
	...
	<xsl:template name="Insured" match="BBR">
		<Insured>
			...
			<!-- 地址 -->
			<Address>
				<xsl:value-of select="substring-after(BBR_ADDR,'#')" />
			</Address>
			<!-- 邮编 -->
			<ZipCode>
	...
	<!-- 缴费频次 -->
	<!-- 银行：0 不定期，1 年交，2 半年交，3 季交，4 月交，6 趸交 -->
	...
	<xsl:template match="CHNL_CODE">
		<xsl:choose>
			...
			<xsl:when test=".='1104'">8</xsl:when><!-- 自助终端 -->
			<xsl:when test=".='1105'">8</xsl:when><!-- 自助终端 -->
		</xsl:choose>
	</xsl:template>
	
	<!-- 争议处理方式 -->
	<xsl:template match="DEALTYPE">
		<xsl:choose>
			<xsl:when test=".='1'">2</xsl:when><!-- 诉讼 -->
			<xsl:when test=".='2'">1</xsl:when><!-- 仲裁 -->
		</xsl:choose>
	</xsl:template>
