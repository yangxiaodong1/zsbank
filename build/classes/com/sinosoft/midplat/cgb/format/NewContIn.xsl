...
<xsl:stylesheet version="1.0"
    ...
    <!--yxd 上面这3个属性不知道干嘛的  -->
	<xsl:template match="/INSUREQ"> <!-- yxd匹配 /INSUREQ 模块  谁和这个 INSUREQ 匹配    --><!-- 这里面的就是保险公司需要的格式吗 -->
		<TranData>    <!-- yxd 数据转换标签 -->
		
			<!-- 报文头 -->
			<xsl:apply-templates select="MAIN" />   <!-- select 这个代表选中吗？？  select 是与下面关联的match的 -->

			<!-- 报文体 -->
			<Body>
				<!-- 投保单(印刷)号 -->
				<ProposalPrtNo>
					<xsl:value-of select="MAIN/APPLNO" /> <!-- APPLNO 这在下面找不到呀 是怎么match的？？？ -->
				</ProposalPrtNo>
				<!-- 保单合同印刷号 -->
				<ContPrtNo></ContPrtNo> <!-- 报文中解释为 保单合同印刷号 重空？？？ -->
				<!-- 投保日期 -->
				<PolApplyDate>
					<xsl:value-of select="MAIN/TRANSRDATE" /> <!-- 这个value-of xsl值从哪个地方去匹配 -->
				</PolApplyDate>
				<!-- 账户姓名 -->
				<AccName>
					<xsl:value-of select="ACCOUNT_INFO/PAY_IN_ACC_NAME" /><!-- 这些select都到那里去配置呀 -->
				</AccName>
				<!-- 银行账户 -->
				<AccNo>
				...
				<!-- 职业告知(N/Y) -->
				<JobNotice>
					<xsl:value-of select="OCCUPATION_NOTICE/NOTICE_ITEM" />
				</JobNotice>
				<!-- 健康告知(N/Y)  -->
				<HealthNotice>
				...
				<!-- 销售员工号 -->
				<SellerNo><xsl:value-of select="MAIN/MANAGERNO" /></SellerNo>
				<!-- 销售员工姓名 -->
				<TellerName><xsl:value-of select="MAIN/MANAGERNAME" /></TellerName>
				<!-- 网点名称 -->
				<AgentComName><xsl:value-of select="MAIN/BRANCHNAME" /></AgentComName>
		
				<!-- 产品组合 -->
				<ContPlan>                 <!-- 报文中    在保单查询中有  在新单试算中的报文中是没有的呀？？？？？？？？-->
					<!-- 产品组合编码 -->
					<ContPlanCode>
						<xsl:apply-templates select="PRODUCTS/PRODUCT[MAINSUBFLAG='0']/PRODUCTID"  mode="plan"/>
					</ContPlanCode>
					<!-- 产品组合份数 -->
					<ContPlanMult>
						<xsl:value-of select="PRODUCTS/PRODUCT[MAINSUBFLAG='0']/AMT_UNIT" />
					</ContPlanMult>
				</ContPlan>
				<!-- 投保人 -->
	...
	<xsl:template name="Appnt" match="TBR">
		<Appnt>
			...
			<!-- 年收入 -->
			<Salary><xsl:value-of select="TBR_AVR_SALARY" /></Salary>
			<!-- 家庭年收入 -->
			<FamilySalary></FamilySalary>
			<!-- 客户类型 -->
			<LiveZone><xsl:apply-templates select="TBR_RESIDENTSOURCE" /></LiveZone>
			<!-- 国籍 -->
			<Nationality>
