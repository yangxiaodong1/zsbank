...
<TXLife>
	...
	<xsl:if test="Head/Flag='0'">
		...
		<!-- 保单单证号 -->
		<ProviderPolicyNumber><xsl:value-of select ="Body/ContPrtNo"/></ProviderPolicyNumber>
		<!-- 收据单证号 -->
		<ProviderFormNumber>无</ProviderFormNumber>
		<!-- 保单号 -->
		...
		<!--大写的合计保费(主险保费＋附加险保费) -->
		<PaymentAmtUper><xsl:value-of select ="Body/PremText"/></PaymentAmtUper>
		...
		<!-- 大写的主险保费，无附加险-->
		<ModalPremAmtUper><xsl:value-of select ="Body/PremText"/></ModalPremAmtUper>
		<!-- 险种名称-->
		...
		<!-- 投保金额：银行方单位万元 -->
		<IntialNumberOfUnits><xsl:value-of select ="java:com.sinosoft.midplat.common.NumberUtil.fenToYuan(Body/Risk/Amnt)*0.0001"/></IntialNumberOfUnits>
		<!-- 投保年龄 -->
		<AgeTouBao></AgeTouBao>
		<!-- 缴费方式名称 -->
		<PaymentModeName>
		...
		<!-- 银管员代码 -->
		<BankManagerCode>无</BankManagerCode>
		<!-- 银管员名称 -->
		<BankManagerName>无</BankManagerName>		
		<!-- 领取年龄 -->
		...
		<!-- 缴费起始日期,此处传贷款起始日期 -->
		<PaymentDueDate><xsl:value-of select ="java:com.sinosoft.midplat.common.DateUtil.date10to8(Body/LoanStartDate)"/></PaymentDueDate>
		<!-- 缴费终止日期,此处传贷款终止日期 -->
		<FinalPaymentDate><xsl:value-of select ="java:com.sinosoft.midplat.common.DateUtil.date10to8(Body/LoanEndDate)"/></FinalPaymentDate>
		<!-- 责任起始日期,保险期限起始日 -->
		...
		<ContractInfo>
			<!--保单保险金额-->
			<RiskAmt><xsl:value-of select ="java:com.sinosoft.midplat.common.NumberUtil.fenToYuan(Body/Risk[RiskCode=MainRiskCode]/Amnt)"/></RiskAmt>
			<!--保单累计保险金额-->
			<TotalRiskAmt><xsl:value-of select ="java:com.sinosoft.midplat.common.NumberUtil.fenToYuan(Body/Risk[RiskCode=MainRiskCode]/Amnt)"/></TotalRiskAmt>
			<!--贷款受益金额描述-->
			<LoanBenefitDesc><xsl:value-of select ="java:com.sinosoft.midplat.common.NumberUtil.fenToYuan(Body/LoanMoney)"/></LoanBenefitDesc>
			<!--保险合同校验码，如果没有校验码就把合同号返回去-->
			<LoanContVerifyNo><xsl:value-of select ="Body/ContNo"/></LoanContVerifyNo>
			<!--贷款合同编码-->
			<LoanContractNo><xsl:value-of select ="Body/ContractNo"/></LoanContractNo>
			<!--贷款凭证编码-->
			<LoanInvoiceNo><xsl:value-of select ="Body/VoucherNo"/></LoanInvoiceNo>
			<!--贷款起期-->
