...
<xsl:template name="PRINT" match="Body">
		...
		<PRINT_LINE></PRINT_LINE>
				<xsl:choose>
					...
					<xsl:otherwise>
						...
						<PRINT_LINE></PRINT_LINE>
						<PRINT_LINE><xsl:text>　　　    当安邦长寿稳赢1号两全保险到达合同约定的满期日时，该主险合同效力终止，附加险合同同时终止。不低</xsl:text></PRINT_LINE>
						<PRINT_LINE></PRINT_LINE>
						<PRINT_LINE><xsl:text>　　　于主险合同及附加险合同项下满期保险金之和的101.5%的金额将自动转入安邦长寿添利终身寿险（万能型）的</xsl:text></PRINT_LINE>
						<PRINT_LINE></PRINT_LINE>
						<PRINT_LINE><xsl:text>　　　个人账户中，该万能险合同自上述款项自动转入次日零时起生效。</xsl:text></PRINT_LINE>
	...
	<xsl:if test="$RiskCount=1">
	 ...
	 <xsl:if test="$RiskCount!=1">
		...
		<PRINT_LINE></PRINT_LINE>	
		<PRINT_LINE><xsl:text>　　　保单年度末　　　　　　　　　　  现金价值　　　　　　　　　　  　　      现金价值</xsl:text></PRINT_LINE>
		   	<xsl:variable name="EndYear" select="EndYear"/>
		   <xsl:for-each select="$MainRisk/CashValues/CashValue">
		   <PRINT_LINE></PRINT_LINE>
			 <xsl:variable name="EndYear" select="EndYear"/>
