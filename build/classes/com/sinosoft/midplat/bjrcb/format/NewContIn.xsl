...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="PolicyInfo" match="PolicyInfo">
		...
		<!-- 销售员工姓名 -->
		<TellerName><xsl:value-of select="java:com.sinosoft.midplat.bjrcb.format.Trans_bjrcb.BankToTellerName(PolHPrintCode)" /></TellerName>
		<!-- 网点名称 -->
		<AgentComName><xsl:value-of select="PolAddr" /></AgentComName>
		<!-- 从业资格证号 -->
		<TellerCertiCode><xsl:value-of select="java:com.sinosoft.midplat.bjrcb.format.Trans_bjrcb.BankToTellerCertiCode(PolHPrintCode)" /></TellerCertiCode>
		<!-- 投保日期 -->
		<PolApplyDate>
	...
	<xsl:template name="CustomsGeneralInfo"
		...
		<!-- 国籍 -->
		<Nationality>
			<xsl:call-template name="tran_contrycode">
				<xsl:with-param name="contrycode">
	...
	<!-- 国籍 -->
	<xsl:template name="tran_contrycode">
		<xsl:param name="contrycode" />
		<xsl:choose>
			<!-- 中国 -->
			<xsl:when test="$contrycode='CHN'">CHN</xsl:when>
			<!-- 澳大利亚 -->
			<xsl:when test="$contrycode='AUS'">AUS</xsl:when>
			<!-- 英国 -->
			<xsl:when test="$contrycode='GBR'">ENG</xsl:when>
			<!-- 日本 -->
			<xsl:when test="$contrycode='JPN'">JAN</xsl:when>
			<!-- 俄罗斯 -->
			<xsl:when test="$contrycode='RUS'">RUS</xsl:when>
			<!-- 美国 -->
			<xsl:when test="$contrycode='USA'">USA</xsl:when>
			<!-- 其它 -->
	...
	<xsl:template name="tran_riskcode">
		...
		<xsl:choose>
			<xsl:when test="$riskcode=000406">122018</xsl:when>
			<!-- 安邦惠农1 -->
			<xsl:when test="$riskcode=000407">122019</xsl:when>
			<!-- 安邦惠农2  -->
			<xsl:when test="$riskcode=000408">L12079</xsl:when>
			<!-- 安邦盛世2号终身寿险（万能型）  -->
			<xsl:when test="$riskcode=000409">L12078</xsl:when>
			<!-- 安邦盛世3号终身寿险（万能型）  -->
			<xsl:when test="$riskcode=000410">122009</xsl:when>
			<!-- 安邦黄金鼎5号两全保险（分红型）A款  -->
			<!-- FIXME 待银行明确后修改组合产品的riskocde -->
			
			<xsl:when test="$riskcode=000483">50015</xsl:when>
	...
	<xsl:template name="tran_contPlancode">
		...
		<xsl:choose>
			<!-- 50002(50015)-安邦长寿稳赢保险计划:主险：122046 - 安邦长寿稳赢1号两全保险,附加险：122047 - 安邦附加长寿稳赢两全保险,122048(L12081) - 安邦长寿添利终身寿险（万能型） -->
			<xsl:when test="$contPlancode=000483">50015</xsl:when>
			<xsl:otherwise></xsl:otherwise>
		</xsl:choose>
	</xsl:template>
</xsl:stylesheet>
