	...
	<xsl:template name="DETAIL_LIST" match="Body">
		<DETAIL_LIST>
			...
			<DETAIL>
				...
				<DETAIL_PRNT/>
				<DETAIL_PRNT/>
				<DETAIL_PRNT><xsl:text>　　　保险单号码：</xsl:text><xsl:value-of select="ContNo" /><xsl:text>                                                    币值单位：人民币元</xsl:text></DETAIL_PRNT>
					...
					<DETAIL_PRNT>
						...
						<xsl:choose>
							<xsl:when test="PayIntv = 0">
								<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_('一次交清', 13)"/>
							</xsl:when>
				...
				<DETAIL_PRNT>　　　------------------------------------------------------------------------------------------------</DETAIL_PRNT>
				<DETAIL_PRNT/>
				<DETAIL_PRNT>　　　保险费合计：<xsl:value-of select="ActSumPremText" />（RMB<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fenToYuan(ActSumPrem)"/>元整）</DETAIL_PRNT>
				<DETAIL_PRNT>　　　------------------------------------------------------------------------------------------------</DETAIL_PRNT>
				<DETAIL_PRNT/>
				<DETAIL_PRNT/>
				<DETAIL_PRNT/>
				...
				<DETAIL_PRNT>　　　保险单特别约定：<xsl:choose>
			...
			<xsl:choose>
				<xsl:when test="$MainRisk/CashValues/CashValue != ''"> <!-- 当有现金价值时，打印现金价值 -->
					...
					<CLASS_COUNT><xsl:value-of select="$RiskCount" /></CLASS_COUNT>
					<MSG>
						...
						<DETAIL_MSG/>
						<DETAIL_MSG>　　　保险单号码：<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(ContNo, 60)"/>币值单位：人民币元 </DETAIL_MSG>
